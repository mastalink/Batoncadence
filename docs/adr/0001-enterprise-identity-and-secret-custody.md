...
BatonCadence will model human Users, Organization Memberships, Saved Instances, Sessions, and Device Credentials separately from agent bearer tokens. Human authentication is federated through OIDC first, then SAML or a trusted-header Adapter; LDAP directories normally connect through an enterprise identity broker, with direct LDAPS reserved for fully self-hosted deployments. External groups grant nothing until an Organization Role Mapping translates them to BatonCadence roles and scopes.
